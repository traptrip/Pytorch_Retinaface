...
from .rcnn_torch.retinaface import RetinaFace
from .rcnn_torch.layers.functions.prior_box import PriorBox
from .utils.box_utils import decode, decode_landm
from .utils.nms.py_cpu_nms import py_cpu_nms
from .utils import face_align

# with open(os.path.join(os.path.dirname(os.path.abspath(__file__)), "config.json"), "r") as read_file:
...
class RetinaDetector:
    def __init__(self, config):
        ...
        self.model_config = self.config['detector']['cfg_re50']
    ...
    def load_model(self, model):
        ...
        if not self.config['gpu']:
            pretrained_dict = torch.load(self.model_path, map_location=lambda storage, loc: storage)
        else:
            device = torch.cuda.current_device()
            pretrained_dict = torch.load(self.model_path, map_location=lambda storage, loc: storage.cuda(device))
    ...
    def detect(self, image):
        ...
        scale = torch.Tensor([np_img.shape[1], np_img.shape[0], np_img.shape[1], np_img.shape[0]])
        np_img -= (104, 117, 123)
        ...
        landmarks = decode_landm(landmarks.data.squeeze(0), prior_data, self.model_config['variance'])
        scale1 = torch.Tensor([np_img.shape[3], np_img.shape[2], np_img.shape[3], np_img.shape[2],
                               np_img.shape[3], np_img.shape[2], np_img.shape[3], np_img.shape[2],
                               np_img.shape[3], np_img.shape[2]])
    ...
    def get_faces(self, img):
        ...
        align_faces = []
        face_landmarks = []
        for x, l in zip(dets, np.reshape(landmarks, (landmarks.shape[0], 5, 2))):
            if x[4] > self.config['visualise_threshold']:
                coordinate_faces.append([int(x[0]), int(x[1]), int(x[2]), int(x[3])])
                align_faces.append(face_align.norm_crop(img, l))
                face_landmarks.append(l)
        self.log.info(f'Retina detect {len(align_faces)} faces')
        return coordinate_faces, face_landmarks, align_faces
